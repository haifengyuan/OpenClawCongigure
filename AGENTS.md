...
- 如果经验只对某个项目有意义，优先写入该项目文档或 `kb/`，不要把一次性噪音塞进长期记忆。
- **没写下来 = 没记住**：不允许只在回复里口头总结，不落文件。
## Harness 落地工作流（2026-03-25）

### 新增执行偏好
1. **先定信源，再行动**：开始任务前先确认哪份文档/哪个文件/哪个系统是 source of truth。
2. **先定验证，再实施**：动手前先想清楚完成后如何验证；没有验证路径的任务，不算真正开始。
3. **优先最小可验证闭环**：先做最小修改并验证，不直接铺开大改。
4. **规则下沉**：发现高频规则时，优先沉到 Skill、脚本、模板、检查表、schema，而不是继续往 prompt 堆字。
5. **留痕强制化**：关键结论、失败原因、交付证据、经验教训必须落到可检索文件。

### 判断规则
- 只是“提醒 Agent 注意” → 写认知文件
- 需要“确保 Agent 做不到违规” → 做成脚本/门禁/校验
- 多处都在描述同一件事 → 收敛到单一信源
- 任务可拆且超过 2 分钟 → 子代理/异步化
